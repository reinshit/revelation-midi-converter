...
export class BrowserMidiPlayer {
  ...
  private soundFontBytes: ArrayBuffer | undefined;
  ...
  async play(
    ...
    const notes = snapshot.tracks.flatMap((track) =>
      track.playback_events
        .filter(
          (event): event is Extract<PlaybackEvent, { type: 'note' }> =>
            event.type === 'note',
        )
    ...
    this.duration = notes.reduce(
      (maximum, note) =>
        Math.max(
          maximum,
          secondsAt(
            note.position_units + note.duration_units,
            tempoMap,
            snapshot.options.smallest_unit,
          ),
        ),
      0,
    );
    this.highlights = (
      snapshot.tracks[highlightTrackIndex]?.playback_events ?? []
    )
      .filter(
        (event): event is Extract<PlaybackEvent, { type: 'note' }> =>
          event.type === 'note',
    ...
    if (synth) {
      for (const track of snapshot.tracks) {
        const programs = track.playback_events
          .filter(
            (event): event is Extract<PlaybackEvent, { type: 'program' }> =>
              event.type === 'program',
          )
          .sort((left, right) => left.position_units - right.position_units);
        const initial = programs.findLast(
          (event) =>
            secondsAt(
        ...
        const channel =
          (initial?.channel ??
            programs[0]?.channel ??
            track.instrument.channel) % 16;
        const program =
          initial?.program ?? programs[0]?.program ?? track.instrument.program;
        synth.midiChannels[channel]?.setDrums(channel === 9);
        synth.programChange(channel, program, { time: now });
        for (const event of programs) {
          const eventTime = secondsAt(
            event.position_units,
            tempoMap,
            snapshot.options.smallest_unit,
          );
          if (eventTime <= this.pausedAt) continue;
          synth.programChange(event.channel % 16, event.program, {
            time: now + eventTime - this.pausedAt,
          });
        }
      }
    }
    for (const note of notes) {
      const noteStart = secondsAt(
        note.position_units,
        tempoMap,
        snapshot.options.smallest_unit,
      );
      const noteEnd = secondsAt(
        note.position_units + note.duration_units,
        tempoMap,
        snapshot.options.smallest_unit,
      );
      ...
      if (synth) {
        synth.noteOn(
          note.channel,
          note.key,
          Math.round((note.velocity / 15) * 127),
          { time: start },
        );
  ...
  async resume() {
    if (this.snapshot && this.listener)
      await this.play(this.snapshot, this.listener, this.highlightTrackIndex);
  }
  ...
  private async getSynth() {
    ...
    this.synthPromise ??= (async () => {
      ...
      if (!this.soundFontBytes) {
        ...
        if (!response.ok)
          throw new Error(`SoundFont failed to load (${response.status}).`);
        this.soundFontBytes = await response.arrayBuffer();
      }
      await synth.soundBankManager.addSoundBank(
        this.soundFontBytes.slice(0),
        'reve-midi-gm',
      );
      await synth.isReady;
  ...
  private scheduleFallbackNote(
    ...
    envelope.gain.linearRampToValueAtTime(
      level,
      start + Math.min(0.01, duration / 4),
    );
    envelope.gain.setValueAtTime(
      level,
      start + Math.max(0.01, duration - 0.02),
    );
...
export function buildTempoMap(snapshot: SongSnapshot): TempoPoint[] {
  ...
  for (const event of snapshot.tracks.flatMap(
    (track) => track.playback_events,
  )) {
