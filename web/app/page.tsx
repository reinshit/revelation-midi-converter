...
import type { SongOptions, SongSnapshot, TrackSnapshot } from '@/lib/wasm/types';
import { defaultSongOptions } from '@/lib/wasm/types';
import { downloadMml, downloadProject, readProject } from '@/lib/storage/project';
...
export default function Home() {
  ...
  useEffect(() => {
    void loadRecentProject().then(setRecentProject).catch(() => undefined);
    void loadKeymap('default').then((mapping) => {
  ...
  async function loadMidi(selectedFile: File) {
    ...
    try {
      ...
      const result = await converter.current.load(await selectedFile.arrayBuffer(), options);
      setFile(selectedFile);
      setSourceName(selectedFile.name);
      applySnapshot(result, 0);
      await saveRecentProject(selectedFile.name, converter.current.getProjectState());
    } catch (cause) {
  ...
  async function restoreProjectState(project: RestoredProject) {
    converter.current?.dispose();
    converter.current = new ConverterWorkerClient();
    const result = await converter.current.restore(project.sourceBytes, project.initialOptions, project.mutations);
    setFile(undefined);
  ...
  function selectTrack(track: TrackSnapshot) {
    ...
    setComparisonTrack((current) => current === track.index ? tracks.find((candidate) => candidate.index !== track.index)?.index ?? 0 : current);
  }

  function applySnapshot(result: SongSnapshot, requestedIndex = activeTrack) {
    const nextIndex = Math.min(requestedIndex, Math.max(0, result.tracks.length - 1));
    setSnapshot(result);
    setOptions(result.options);
    setActiveTrack(nextIndex);
    setComparisonTrack((current) => current < result.tracks.length && current !== nextIndex
      ? current
      : result.tracks.find((track) => track.index !== nextIndex)?.index ?? nextIndex);
  }
  ...
  async function runOperation(
    ...
    try {
      ...
      if (sourceName) await saveRecentProject(sourceName, converter.current.getProjectState());
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : String(cause));
    } finally {
      setBusy(false);
  ...
  async function updateOptions(patch: Partial<SongOptions>) {
    ...
    if (converter.current) await runOperation((client) => client.updateOptions(next));
  }

  function renameSelected() {
    const name = window.prompt('Track name', selected.name)?.trim();
    if (name) void runOperation((client) => client.renameTrack(activeTrack, name));
  }
  ...
  function updateProgress(nextPosition: number, nextDuration: number, range?: PlaybackRange) {
  ...
  async function applyKeymap() {
    try {
      ...
      if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('The keymap must be a JSON object');
      const mapping: Record<number, number> = {};
      for (const [fromText, toValue] of Object.entries(raw)) {
        const from = Number(fromText);
        if (!Number.isInteger(from) || from < 0 || from > 127 || !Number.isInteger(toValue) || Number(toValue) < 0 || Number(toValue) > 127) {
  ...
  async function clearLocalData() {
    if (!window.confirm('Clear saved projects, keymaps, and preferences from this browser? The currently open project will remain available until this tab is closed.')) return;
    try {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6">
        <div
          className="mb-5 flex flex-col gap-3 rounded-2xl border bg-card p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between"
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            ...
            />
            <input ref={projectInput} type="file" accept=".json,application/json" aria-label="Project file" className="sr-only" onChange={(event) => { const selected = event.target.files?.[0]; if (selected) void restoreProject(selected); }} />
            ...
            <Button variant="outline" size="icon" title="Save project" disabled={!snapshot} onClick={saveProject}><Save className="size-4" /></Button>
            <Button variant="outline" size="icon" title="Export all MML" disabled={!snapshot} onClick={() => snapshot && downloadMml(sourceName ?? 'song.mid', snapshot.tracks)}><Download className="size-4" /></Button>
          </div>
        </div>

        {error ? (
          <p role="alert" className="mb-4 rounded-xl border border-destructive/25 bg-destructive/10 px-4 py-3 text-sm text-destructive">
            Something went wrong: {error}
          </p>
        ) : null}

        <div className="grid gap-4 lg:grid-cols-[250px_minmax(0,1fr)_290px]">
          <Card className="h-fit gap-0 overflow-hidden py-0">
            <CardHeader className="flex-row items-center justify-between border-b px-4 py-3">
              <CardTitle className="text-sm">Track</CardTitle><Badge variant="outline">{tracks.length}</Badge>
            </CardHeader>
            <CardContent className="space-y-1 p-2">
              {tracks.map((track) => (
                <button
                  ...
                  className={`w-full rounded-xl border px-3 py-3 text-left transition-colors ${activeTrack === track.index ? 'border-primary/20 bg-primary/8' : 'border-transparent hover:bg-muted'}`}
                >
                  <span className="flex justify-between gap-2 text-sm font-medium">
                    <span className="truncate">{track.name}</span>
                    <span className="font-mono text-[10px] text-muted-foreground">{String(track.index + 1).padStart(2, '0')}</span>
                  </span>
                  <span className="mt-1 block truncate text-xs text-muted-foreground">{track.instrument.name}</span>
                  <span className="mt-2 block text-[11px] text-muted-foreground">{track.mml_note_length} MML notes</span>
                </button>
              ))}
            </CardContent>
          </Card>
          ...
          <Card className="min-w-0 gap-0 overflow-hidden py-0">
            <CardHeader className="flex-row items-center justify-between border-b px-4 py-3">
              <div className="min-w-0">
                <CardTitle className="truncate text-sm">{selected.name}</CardTitle>
                <p className="mt-1 truncate text-xs text-muted-foreground">{selected.instrument.name}</p>
              </div>
              <div className="flex gap-1">
                <Button variant="ghost" size="icon" title="Rename track" disabled={!snapshot || busy} onClick={renameSelected}><Pencil className="size-4" /></Button>
                <Button variant="ghost" size="icon" title="Split track" disabled={!snapshot || busy} onClick={() => void runOperation((client) => client.splitTrack(activeTrack))}><Scissors className="size-4" /></Button>
                <Button variant="ghost" size="icon" title="Merge selected tracks" disabled={!snapshot || tracks.length < 2 || busy} onClick={() => void runOperation((client) => client.mergeTracks(activeTrack, comparisonTrack), Math.min(activeTrack, comparisonTrack))}><Merge className="size-4" /></Button>
                <Button variant="ghost" size="icon" title="Equalize selected tracks" disabled={!snapshot || tracks.length < 2 || busy} onClick={() => void runOperation((client) => client.equalizeTracks(activeTrack, comparisonTrack))}><Scale className="size-4" /></Button>
                <Button variant="ghost" size="icon" title="Copy MML" onClick={() => navigator.clipboard?.writeText(selected.mml)}><Copy className="size-4" /></Button>
              </div>
            </CardHeader>
            <CardContent className="p-4">
              {playing ? (
                <pre aria-label="MML code with playback highlighting" className="min-h-[430px] overflow-auto whitespace-pre-wrap break-all rounded-md bg-editor px-3 py-2 font-mono text-[13px] leading-7 shadow-inner">
                  {activeRange ? <>{selected.mml.slice(0, activeRange.charStart)}<mark className="rounded bg-primary px-0.5 text-primary-foreground">{selected.mml.slice(activeRange.charStart, activeRange.charEnd)}</mark>{selected.mml.slice(activeRange.charEnd)}</> : selected.mml}
                </pre>
              ) : (
                <Textarea
                  aria-label="MML code"
                  ...
                  readOnly
                  spellCheck={false}
                  className="min-h-[430px] resize-none border-0 bg-editor font-mono text-[13px] leading-7 shadow-inner"
                />
              )}
              <div className="mt-3 flex justify-between text-xs text-muted-foreground">
                <span>{selected.mml.length} characters</span><span>{selected.mml_note_length} notes</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="h-fit gap-0 overflow-hidden py-0">
            <CardHeader className="flex-row items-center gap-2 border-b px-4 py-3">
              <Settings2 className="size-4" /><CardTitle className="text-sm">Song options</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6 p-4">
              <label className="block text-sm font-medium" htmlFor="comparison-track">Second track</label>
              <select id="comparison-track" value={comparisonTrack} disabled={!snapshot || tracks.length < 2 || busy} onChange={(event) => setComparisonTrack(Number(event.target.value))} className="w-full rounded-md border bg-background px-3 py-2 text-sm">
                {tracks.filter((track) => track.index !== activeTrack).map((track) => <option key={track.index} value={track.index}>{track.index + 1}. {track.name}</option>)}
              </select>
              ...
              <Button variant="ghost" className="w-full text-destructive" onClick={() => void clearLocalData()}><Trash2 className="size-4" /> Clear local data</Button>
            </CardContent>
          </Card>
        </div>
      </section>
      ...
      <Dialog open={keymapOpen} onOpenChange={setKeymapOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Enter source and destination MIDI key pairs as JSON.</DialogDescription>
          </DialogHeader>
          <Textarea value={keymapText} onChange={(event) => setKeymapText(event.target.value)} className="min-h-48 font-mono" spellCheck={false} aria-label="Keymap JSON" />
          <DialogFooter>
            <Button variant="outline" onClick={() => setKeymapOpen(false)}>Cancel</Button>
...
function OptionSlider({ label, value, values, min, max, step, onChange }: { label: string; value: string; values: number[]; min: number; max: number; step: number; onChange: (values: number[]) => void }) {
  return (
    <div>
      <div className="mb-3 flex justify-between text-sm"><span className="font-medium">{label}</span><span className="font-mono text-xs text-muted-foreground">{value}</span></div>
      <Slider aria-label={label} value={values} min={min} max={max} step={step} onValueChange={(next) => onChange(typeof next === 'number' ? [next] : Array.from(next))} />
    </div>
  );
}
