...
test('converts, edits, highlights, exports, and restores a MIDI project', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(page.getByText('Lead track', { exact: true }).first()).toBeVisible();
  ...
  await expect(page.getByLabel('MML code with playback highlighting')).toBeVisible();
...
test('starts and stops SoundFont playback for every golden MIDI fixture', async ({ page }) => {
  await page.goto('/');
...
test('finishes a short tempo-changing song and returns to a stopped state', async ({ page }) => {
  await page.goto('/');
